...
namespace MovementEffects
{
    public class Timing : MonoBehaviour
    {
        ...
        private void RemoveUnused()
        {
            ...
            {
                if (waitTrigsEnum.Current.Value.Tasks.Count == 0)
                {
                    if (_handleToIndex.ContainsKey(waitTrigsEnum.Current.Key))
                        CoindexReplace(_handleToIndex[waitTrigsEnum.Current.Key], waitTrigsEnum.Current.Value.Coroutine);
                ...
                if (!_pausedProcessBuckets.ContainsKey(waitTrigsEnum.Current.Key) && _handleToIndex.ContainsKey(waitTrigsEnum.Current.Key) && 
            ...
            if (_handleToIndex.ContainsKey(otherCoroutine))
            {
                ...
                if (!_waitingTriggers.ContainsKey(otherCoroutine))
                {
                    WaitingProcess newWaitingProcess = new WaitingProcess 
                        { Coroutine = CoindexReplace(_handleToIndex[otherCoroutine], _StartWhenDone(otherCoroutine)) };
            ...
            if (_pausedProcessBuckets.ContainsKey(otherCoroutine))
            {
                ...
                while(tasksEnum.MoveNext())
                {
                    if (tasksEnum.Current.Handle == otherCoroutine)
                    {
                        if (!_waitingTriggers.ContainsKey(otherCoroutine))
                        {
                            WaitingProcess newWaitingProcess = new WaitingProcess { Coroutine = tasksEnum.Current.Coroutine };
                            tasksEnum.Current.Coroutine = _StartWhenDone(otherCoroutine);
                            _waitingTriggers.Add(otherCoroutine, newWaitingProcess);
                        }
        ...
        private void CloseWaitingProcess(CoroutineHandle handle)
        {
            ...
            var tasksEnum = _waitingTriggers[handle].Tasks.GetEnumerator();
